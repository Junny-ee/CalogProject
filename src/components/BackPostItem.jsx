...
import { BackBoardDispatchContext } from './BackBoard';
import { useContext } from "react";
// 태그 여러 개 구현 필요
const BackPostItem = ({ id, title, createDate, content, tag }) => {
  const nav = useNavigate();
  const { deleteContent, setSearchWord, setSearchingTag, setShowSearchBar } = useContext(BackBoardDispatchContext);
  return (
    <div className="post_item">
      ...
      <button className="button_delete" onClick={() => deleteContent(id)}>삭제하기 테스트 버튼(현재 로컬 스토리지가 없어서 새로고침/페이지 이동하면 복구됨)</button>
    </div>
  );
};
