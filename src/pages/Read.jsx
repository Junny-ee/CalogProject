...
const Read = ({ title, createDate, content, tag }) => {
  const targetContent = postContent.find(
    (content) => Number(content.id) === Number(params.id)
  );
  ...
  return (
    <>
      ...
      <div className="read_createDate">
        {new Date(targetContent.createDate).toLocaleDateString()}
      </div>
